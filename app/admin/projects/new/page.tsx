  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const createResponse = await fetch("/api/projects", {
        ...
        body: JSON.stringify(buildPayload({ img: "/placeholder.svg", imagePublicId: "" })),
      })
      const createPayload = await createResponse.json()

      if (!createResponse.ok || !createPayload?.success || !createPayload?.project?.id) {
        throw new Error(createPayload?.message || "Project yaratishda xatolik bo'ldi.")
      }

      const createdProjectId = Number(createPayload.project.id)
      if (!Number.isInteger(createdProjectId) || createdProjectId <= 0) {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-2xl font-bold">Yangi project qo&apos;shish</h1>
        <Link
          href="/admin/projects"
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 rounded-2xl border border-border bg-card/30 p-5">
        ...
        <div className="grid gap-2">
          <span className="text-sm font-medium">Project name (3 ta til)</span>
        ...
        <label className="grid gap-2">
          ...
          />
          {localPreviewUrl ? (
            <img src={localPreviewUrl} alt="Selected preview" className="h-24 w-40 rounded-md border border-border object-cover" />
          ) : null}
        ...
        <div className="grid gap-2">
          <span className="text-sm font-medium">Description (3 ta til)</span>
          <div className="grid gap-3 md:grid-cols-3">
        ...
        <div className="grid gap-2">
          <span className="text-sm font-medium">Badge (optional, 3 ta til)</span>
          <div className="grid gap-3 md:grid-cols-3">
        ...
        <label className="grid gap-2">
          ...
          <input type="url" value={form.github} onChange={(e) => setForm((prev) => ({ ...prev, github: e.target.value }))} className="rounded-md border border-border bg-background px-3 py-2 text-sm" placeholder="https://github.com/username/repo" />
        </label>

        <label className="grid gap-2">
          <span className="text-sm font-medium">Admin URL (optional)</span>
          <input type="url" value={form.admin} onChange={(e) => setForm((prev) => ({ ...prev, admin: e.target.value }))} className="rounded-md border border-border bg-background px-3 py-2 text-sm" placeholder="https://example.com/admin" />
        </label>
        ...
        <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-50">
          {isSubmitting ? "Saqlanmoqda..." : "Projectni saqlash"}
        </button>
      </form>
    </section>
  )
